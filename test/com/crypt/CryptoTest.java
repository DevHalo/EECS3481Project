...
public class CryptoTest {
    ...
    void AESTest() {
        for (int i = 0; i < testFiles.length; i++) {
            ...
            try {
                AES.crypt(fileName.toString(), key.getBytes("UTF-8"), Utilities.ENCRYPT);
            } catch (Exception e) {
                e.printStackTrace();
                fail("Failed to encrypt file " + fileName.toString());
            }
            ...
            try {
                AES.crypt(fileName.toString() + Utilities.ENCRYPTED_EXTENSION, key.getBytes("UTF-8"), Utilities.DECRYPT);
            } catch (Exception e) {
                e.printStackTrace();
                fail("Failed to decrypt file " + fileName.toString());
            }
